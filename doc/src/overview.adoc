...
link:#bbv2.reference.init.options.help[`--help`]).
...
directly to the C++ compiler, and values of the `compileflags` feature
...
|define
|(Arbitrary string)
|Additional macro definitions for C and C++ compilers. The string should be
...
link:#bbv2.reference.rules[???]. The user can also declare custom main
target rules as shown in link:#bbv2.extending.rules[???].
...
link:#bbv2.reference.variants.proprefine[???]
...
link:#bbv2.overview.projects.attributes.projectrule[para_title].
...
link:#bbv2.overview.builtins.features[???]. For example if a library can
only be built statically, or a file can't be compiled with optimization
...
requirementsconditional Sometimes, particular relationships need to be
maintained among a target's build properties. This can be achieved with
conditional requirements. For example, you might want to set specific
...
requirementsindirect A more powerful variant of conditional requirements
is indirect conditional requirements. You can provide a rule that will
...
used to build it. We can express this situation using target
alternatives:
...
Here, attributes is a sequence of rule arguments, each of which begins
...
used in Jamfile see link:#bbv2.reference.rules[???].
...
in link:#bbv2.overview.invocation[section_title].
...
link:#bbv2.reference.buildprocess[???].
