...
requirementsMDASHprimarily to add support for non-standard tools you
...
jam target::
  ...
  Essentially a stringMDASHmost often a name of file.
...
All of the Boost.Jam target-related builtin functions, like `DEPENDS` or
`ALWAYS` operate on jam targets. Applying them to metatargets or
...
* As front elementMDASHUsage-requirements from this invocation (an
instance of link:#bbv2.reference.class.property-set[property-set])
* As subsequent elementsMDASHcreated concrete targets ( instances of the
...
implicitlyMDASHwhen loading a new Jamfiles, or when a new target
...
link:#bbv2.reference.class.basic-target[basic-target]MDASH
...
file target is associated with an action that creates itMDASH an
