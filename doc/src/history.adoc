...
handful of new features. There's initial support for the Fil-C compiler,
the `version` attribute for features to manage versioning of targets,
and additional support for JSON data.
...
* Fix gcc command-db collection not working with multi-string commands.
  -- _René Ferdinand Rivera Morell_
* Make `regex.escape` accept a list of strings to escape instead of just one
  string.
  -- _René Ferdinand Rivera Morell_
...
* Add `libstdc++` to `stdlib` feature as the current naming for that std
  library.
  -- _Dmitry Arkhipov_
* Fix building when targeting WASM64 with emscripten.
  -- _Uilian Ries_
