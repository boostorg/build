...
#endif
static inline bool str_view_equal(
	const char * str_a, std::size_t size_a, const char * str_b, std::size_t size_b)
{
...
static inline int str_view_cmp(
	const char * str_a, std::size_t size_a, const char * str_b, std::size_t size_b)
{
...
{
	type get_type() const override { return type::null; }
	string_view as_string() const override { return { nullptr, 0 }; }
	double as_number() const override
	{
		using nl = std::numeric_limits<double>;
		return nl::has_quiet_NaN ? nl::quiet_NaN() : nl::lowest();
	}
	object * as_object() const override { return nullptr; }
};
...
struct value_str : value_base
{
	...
	bool equal_to(const value & o) const override
	{
		return str_view_equal(
			value, size, o.as_string().data(), o.as_string().size());
	}
	int compare_to(const value & o) const override
	{
		return str_view_cmp(
			value, size, o.as_string().data(), o.as_string().size());
	}
	string_view as_string() const override { return { value, size }; }
	value * to_string() override { return this; }
...
struct value_object : value_base
{
	...
	type get_type() const override { return type::object; }
	bool equal_to(const value & o) const override
	{
	...
	object * as_object() const override { return value.get(); }
	value * to_string() override
	{
...
struct value_str_view : value_base
{
	...
	string_view as_string() const override { return { value, size }; }
	value * to_string() override { return nullptr; }
...
void value::done() { value_cache().reset(); }
